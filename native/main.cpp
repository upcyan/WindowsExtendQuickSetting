...
bool SetEthernetEnabled(bool enabled) {
    ...
    if (GetAdaptersAddresses(AF_UNSPEC, 0, nullptr, addresses, &size) == ERROR_BUFFER_OVERFLOW) {
        buffer.resize(size); addresses = reinterpret_cast<PIP_ADAPTER_ADDRESSES>(buffer.data());
    }
    ...
    for (auto* address = addresses; address; address = address->Next) {
        // Ethernet control must never select a WLAN or USB/RNDIS interface.
        if (address->IfType != IF_TYPE_ETHERNET_CSMACD || !address->FriendlyName ||
        ...
        const bool isEnabled = row.AdminStatus == NET_IF_ADMIN_STATUS_UP;
        // Enable a disabled physical adapter first; when disabling, prefer an
        // enabled adapter. Link state is deliberately not used here: an
        // unplugged Ethernet adapter can still be correctly enabled.
        if (!selected || (enabled && !isEnabled) || (!enabled && isEnabled)) {
            selected = address;
            if ((enabled && !isEnabled) || (!enabled && isEnabled)) break;
        }
    }
    if (!selected) return false;
    const std::wstring name(selected->FriendlyName);
    if (!IsSafeNetshInterfaceName(name)) return false;
    const std::wstring cmd = L"netsh interface set interface name=\"" + name + L"\" admin=" + (enabled ? L"ENABLED" : L"DISABLED");
...
bool ApplyBackdrop() {
    ...
    const int backdropType = g_backdropMica ? 2 : 3;
    const int roundedCorners = 2; // DWMWCP_ROUND
    const auto result = DwmSetWindowAttribute(g_window, 38, &backdropType, sizeof(backdropType));
...
void RefreshBackdrop() {
    ...
    DWORD type{}, size = sizeof(value);
    if (RegOpenKeyExW(HKEY_CURRENT_USER, L"Software\\WindowsExtendQuickSetting.Native", 0, KEY_READ, &key) == ERROR_SUCCESS) {
        if (RegQueryValueExW(key, L"BackdropEffect", nullptr, &type, reinterpret_cast<BYTE*>(value), &size) == ERROR_SUCCESS && type == REG_SZ)
            g_backdropMica = _wcsicmp(value, L"Mica") == 0;
        RegCloseKey(key);
    }
}

bool SetBackdrop(bool mica) {
    HKEY key{};
    if (RegCreateKeyExW(HKEY_CURRENT_USER, L"Software\\WindowsExtendQuickSetting.Native", 0, nullptr, 0,
        KEY_SET_VALUE, nullptr, &key, nullptr) != ERROR_SUCCESS) return false;
    const std::wstring value = mica ? L"Mica" : L"Acrylic";
    const bool saved = RegSetValueExW(key, L"BackdropEffect", 0, REG_SZ, reinterpret_cast<const BYTE*>(value.c_str()),
        static_cast<DWORD>((value.size() + 1) * sizeof(wchar_t))) == ERROR_SUCCESS;
    RegCloseKey(key);
    if (!saved) return false;
    g_backdropMica = mica;
    const bool applied = ApplyBackdrop();
    ShowToast(mica ? Tr(L"已切换为云母背景", L"Backdrop changed to Mica") : Tr(L"已切换为亚克力背景", L"Backdrop changed to Acrylic"));
    return applied;
}
...
void PaintWindow(HWND hwnd) {
    ...
    auto background = CreateSolidBrush(g_backdropMica ? RGB(243, 243, 243) : RGB(238, 244, 248));
    ...
    if (g_page == Page::Settings) {
        ...
        const auto drawSettingsCard = [&](int top, const wchar_t* glyph, const wchar_t* title,
            ...
            FillRoundRect(dc, card, RGB(250, 252, 253), 8);
        ...
        drawSettingsCard(230, L"\xE790", Tr(L"背景特效", L"Backdrop effect"),
            Tr(L"选择亚克力或云母材质", L"Choose Acrylic or Mica material"),
            g_backdropMica ? Tr(L"云母  \xE70D", L"Mica  \xE70D") : Tr(L"亚克力  \xE70D", L"Acrylic  \xE70D"));
        drawSettingsCard(314, L"\xE839", Tr(L"上网方式优先级", L"Connection priority"),
            Tr(L"调整以太网、Wi-Fi、USB 共享的优先级", L"Set Ethernet, Wi-Fi and USB tether priority"),
            Tr(L"管理", L"Manage"));
        drawSettingsCard(398, L"\xE946", Tr(L"关于", L"About"),
        ...
        DrawText(dc, Tr(L"网络与蓝牙快速控制", L"Quick network and Bluetooth controls"),
            78, 449, 240, 22, RGB(105, 105, 105), 11);
    ...
    if (g_page == Page::Doh) {
        ...
        FillRoundRect(dc, RECT{24, 62, 344, 166}, RGB(250, 252, 253), 10);
        ...
        DrawText(dc, std::wstring(Tr(L"首选：", L"Primary: ")) + primary + L"  \xE70D", 40, 124, 142, 24, RGB(55, 55, 55), 11, true);
        DrawText(dc, std::wstring(Tr(L"备用：", L"Backup: ")) + backup + L"  \xE70D", 188, 124, 140, 24, RGB(55, 55, 55), 11, true);

        FillRoundRect(dc, RECT{24, 178, 344, 612}, RGB(250, 252, 253), 10);
        DrawText(dc, Tr(L"DoH 服务器（DNS over HTTPS）", L"DoH servers (DNS over HTTPS)"), 40, 192, 250, 24, RGB(30, 30, 30), 13, true);
        DrawText(dc, Tr(L"快速导入", L"Quick import"), 40, 222, 72, 24, RGB(0, 103, 192), 11, true);
        DrawText(dc, Tr(L"手动添加", L"Manual entry"), 118, 222, 72, 24, RGB(0, 103, 192), 11, true);
        DrawText(dc, Tr(L"搜索", L"Find"), 204, 222, 38, 24, RGB(0, 103, 192), 11, true);
        DrawText(dc, Tr(L"应用", L"Apply"), 246, 222, 38, 24, RGB(0, 103, 192), 11, true);
        if (!g_dohSearch.empty()) DrawText(dc, Tr(L"清除", L"Clear"), 292, 222, 36, 24, RGB(0, 103, 192), 11, true);
        ...
        for (size_t row = 0; row < std::min<size_t>(filteredDoh.size(), 12); ++row) {
            const auto index = filteredDoh[row];
            const int top = 254 + static_cast<int>(row) * 29;
            DrawText(dc, g_dohServers[index].address, 40, top, 92, 18, RGB(55, 55, 55), 11, true);
            DrawText(dc, g_dohServers[index].templateUrl, 132, top, 154, 18, RGB(100, 100, 100), 10);
            DrawText(dc, L"×", 306, top - 1, 20, 20, RGB(130, 50, 50), 14, true);
        }
        if (g_dohPicker != 0) {
    ...
    DrawTile(dc, RECT{24, 34, 123, 81}, L"\xE839", ethernetCaption.c_str(), g_ethernetOn, g_hoverTile == 1 && !g_hoverTileExpand, g_hoverTile == 1 && g_hoverTileExpand,
        g_detailsVisible && g_detailsKind == 3);
    DrawTile(dc, RECT{135, 34, 234, 81}, L"\xE701", wifiCaption.c_str(), g_wifiOn, g_hoverTile == 2 && !g_hoverTileExpand, g_hoverTile == 2 && g_hoverTileExpand,
        g_detailsVisible && g_detailsKind == 1);
    DrawTile(dc, RECT{245, 34, 344, 81}, L"\xE702", bluetoothCaption.c_str(), g_bluetoothOn, g_hoverTile == 3 && !g_hoverTileExpand, g_hoverTile == 3 && g_hoverTileExpand,
        g_detailsVisible && g_detailsKind == 2);
    DrawTile(dc, RECT{24, 104, 123, 151}, L"\xE88E", usbCaption.c_str(), g_usbOn, g_hoverTile == 4 && !g_hoverTileExpand, g_hoverTile == 4 && g_hoverTileExpand,
    ...
    if (g_currentNetworkExpanded) {
        ...
        DrawGlyph(dc, L"\xE713", 306, networkTop + 92, 24, 24, RGB(80, 80, 80), 12);
    } else {
    ...
    if (g_detailsVisible && g_detailsKind == 1) {
        DrawText(dc, Tr(L"可用网络", L"Available networks"), 40, 200, 200, 20, RGB(70, 70, 70), 11, true);
        DrawGlyph(dc, g_wifiAvailableExpanded ? L"\xE70D" : L"\xE70E", 306, 201, 20, 18, RGB(75, 75, 75), 11);
        if (g_wifiAdapterIndex < g_wifiAdapters.size()) {
            const auto& adapter = g_wifiAdapters[g_wifiAdapterIndex];
            const auto adapterLabel = adapter.name + (adapter.connected ? Tr(L" · 已连接", L" · Connected") : Tr(L" · 未连接", L" · Disconnected"));
            DrawText(dc, adapterLabel, 164, 200, 82, 20, RGB(0, 103, 192), 10, true);
            if (g_wifiAdapters.size() > 1) DrawGlyph(dc, L"\xE70D", 246, 201, 16, 18, RGB(0, 103, 192), 10);
        }
        ...
        if (connectedWifi) DrawText(dc, Tr(L"断开", L"Disconnect"), 270, 200, 60, 20, RGB(0, 103, 192), 11, true);
        if (g_wifiAvailableExpanded) {
            ...
            for (size_t index = g_wifiAvailableOffset; index < availableEnd; ++index) {
                ...
                DrawText(dc, Tr(L"连接", L"Connect"), 278, top, 48, 20, RGB(0, 103, 192), 11, true);
            }
        ...
        DrawText(dc, Tr(L"蓝牙适配器", L"Bluetooth adapters"), 40, 200, 260, 20, RGB(70, 70, 70), 11, true);
        ...
        for (size_t index = 0; index < std::min<size_t>(g_bluetoothAdapters.size(), 4); ++index) {
            ...
            DrawText(dc, g_bluetoothAdapters[index].enabled ? Tr(L"禁用", L"Disable") : Tr(L"启用", L"Enable"),
                274, top, 52, 20, RGB(0, 103, 192), 11, true);
        }
        DrawText(dc, Tr(L"蓝牙设备", L"Bluetooth devices"), 40, 306, 180, 20, RGB(70, 70, 70), 11, true);
        DrawText(dc, g_bluetoothScanning ? Tr(L"扫描中…", L"Scanning…") : Tr(L"扫描", L"Scan"), 276, 306, 58, 20,
            g_bluetoothScanning ? RGB(140, 140, 140) : RGB(0, 103, 192), 11, true);
        ...
        DrawText(dc, Tr(L"发送文件", L"Send files"), 40, 440, 110, 24, RGB(0, 92, 175), 12, true);
        DrawText(dc, Tr(L"接收文件", L"Receive files"), 170, 440, 110, 24, RGB(0, 92, 175), 12, true);
        ...
        for (size_t index = 0; index < std::min<size_t>(g_networkAdapters.size(), 10); ++index) {
            ...
            DrawText(dc, Tr(L"编辑", L"Edit"), 278, top, 50, 20, RGB(0, 103, 192), 10, true);
        }
        ...
        if (g_usbInterface.empty()) {
            ...
            DrawText(dc, g_usbOn ? Tr(L"断开", L"Disconnect") : Tr(L"启用", L"Enable"), 274, 228, 52, 20, RGB(0, 103, 192), 11, true);
...
void ToggleWindow() {
    // A tray click only hides an already foreground popup. A visible popup
    // behind another app is brought back to the foreground instead.
    if (IsWindowVisible(g_window) && GetForegroundWindow() == g_window) {
        if (!g_fadeHiding) {
            g_fadeHiding = true;
        ...
        SetWindowPos(g_window, HWND_TOPMOST, work.right - kWindowWidth - 16, work.bottom - height - 8,
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam) {
    ...
    case WM_TIMER:
        if (wparam == kRefreshTimer) {
            ...
            if (difference == 0) {
                ...
                SetWindowPos(hwnd, HWND_TOPMOST, work.right - kWindowWidth - 16, work.bottom - nextHeight - 8,
    ...
    case WM_MOUSEMOVE: {
        ...
        const int hover = y >= 34 && y <= 81 ? (x <= 123 ? 1 : x <= 234 ? 2 : x <= 344 ? 3 : 0) :
            (x >= 24 && x <= 123 && y >= 104 && y <= 151 ? 4 : 0);
        const bool hoverExpand = hover == 1 ? x >= 74 : hover == 2 ? x >= 184 : hover == 3 ? x >= 294 : hover == 4 ? x >= 74 : false;
        const int networkTop = CurrentNetworkTop();
    ...
    case WM_LBUTTONUP: {
        ...
        if (g_page == Page::Settings) {
            ...
            else if (y >= 230 && y <= 302) SetBackdrop(!g_backdropMica);
            else if (y >= 314 && y <= 386) {
                g_page = Page::Home;
        ...
        if (g_page == Page::Doh) {
            ...
            else if (y >= 216 && y <= 248 && x >= 292) g_dohSearch.clear();
            else if (y >= 216 && y <= 248 && x >= 246) {
            ...
            else if (y >= 216 && y <= 248 && x >= 204) g_dohSearch = PromptText(hwnd, Tr(L"搜索系统 DoH", L"Search system DoH"), Tr(L"输入 IP 或模板关键字", L"Enter an IP or template keyword"));
            else if (y >= 216 && y <= 248 && x >= 118) AddDohServer(hwnd);
            else if (y >= 216 && y <= 248 && x >= 40) ImportDohServers(hwnd);
            else if (y >= 254 && y < 612 && x >= 280) {
                const auto row = static_cast<size_t>((y - 254) / 29);
                const auto filtered = FilteredDohIndices();
                if (row < filtered.size()) DeleteDohServer(hwnd, g_dohServers[filtered[row]]);
            }
            ResizeNearTray();
            InvalidateRect(hwnd, nullptr, TRUE);
            return 0;
        ...
        if (y >= 34 && y <= 81) {
        ...
        } else if (y >= 104 && y <= 151 && x >= 24 && x <= 123) {
            if (x < 74) {
        ...
        } else if (g_detailsVisible && g_detailsKind == 1 && y >= 196 && y <= 224 && x >= 164 && x < 270 && g_wifiAdapters.size() > 1) {
            ...
            const int networkTop = CurrentNetworkTop();
            if (y >= networkTop + 82 && y <= networkTop + 126 && x >= 280) {
                g_page = Page::Doh;
                RefreshDohServers();
...
int APIENTRY wWinMain(HINSTANCE instance, HINSTANCE, PWSTR, int) {
    ...
    g_window = CreateWindowExW(WS_EX_TOOLWINDOW | WS_EX_LAYERED, kClassName, kTitle, WS_POPUP | WS_BORDER,
