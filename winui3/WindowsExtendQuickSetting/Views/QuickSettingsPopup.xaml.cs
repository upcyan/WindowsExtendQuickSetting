...
public sealed partial class QuickSettingsPopup : Window
{
    ...
    private Button? _displayExpandButton;
    ...
    private TextBlock? _displayCaption;
    ...
    private bool _bluetoothRefreshInProgress;
    private bool _bluetoothAdapterSnapshotInitialized;
    private string? _activeBluetoothAdapterId;
    ...
    private bool _showDisplayDetails;
    ...
    private Microsoft.UI.Dispatching.DispatcherQueueTimer? _displayRefreshTimer;
    private Button? _displayDuplicateButton;
    private Button? _displayExtendButton;
    private Button? _displayHdrButton;
    ...
    public QuickSettingsPopup()
    {
        ...
        {
            if (_contentScroll == null || _contentScroll.ScrollableHeight <= 0) return;
            var delta = e.GetCurrentPoint(_contentScroll).Properties.MouseWheelDelta;
            if (delta == 0) return;
        ...
        _displayRefreshTimer = DispatcherQueue.CreateTimer();
        _displayRefreshTimer.Interval = TimeSpan.FromSeconds(3);
    ...
    private void SetupWindow()
    {
        ...
        // A fully transparent caption area removes the 1 px bright line DWM draws
        // along the top edge when the title bar is extended into the client area.
        var captionColor = 0x00FFFFFF;
        NativeMethods.DwmSetWindowAttribute(hwnd, 35, ref captionColor, sizeof(int));
        ...
        if (_appWindow != null)
        {
            ...
            if (_appWindow.Presenter is OverlappedPresenter presenter)
            {
                ...
                // SW_MINIMIZE from the tray toggle is only honored when the
                // presenter allows minimizing.
    ...
    public void PositionNearTray()
    {
        try
        {
            ...
            else if (workArea.Right < screen.Right) x = workArea.Right - panelW - 8;
            if (workArea.Top > screen.Top) y = workArea.Top + 8;
            x = Math.Clamp(x, workArea.Left, Math.Max(workArea.Left, workArea.Right - panelW));
            y = Math.Clamp(y, workArea.Top, Math.Max(workArea.Top, workArea.Bottom - panelH));
    ...
    private void BuildUI()
    {
        ...
        var displayState = DisplayService.GetHdrState();
        ...
        _displayButton.Click += (_, _) =>
        {
            ...
            var switched = DisplayService.SwitchMode(extend);
            ShowToast(switched
                ? (extend ? (isZh ? "已切换为扩展屏幕" : "Extended displays enabled") : (isZh ? "已切换为复制屏幕" : "Duplicate displays enabled"))
                : (isZh ? "屏幕模式切换失败" : "Display mode switch failed"));
            if (switched)
            {
                if (_displayCaption != null) _displayCaption.Text = extend ? (isZh ? "扩展" : "Extend") : (isZh ? "复制" : "Duplicate");
                _ = RefreshDisplayVisualAfterDelayAsync();
            }
        };
        _displayExpandButton.Click += (_, _) => ShowDisplayDetails();
        _displayButton.Background = displayState.ActiveDisplays > 0 ? ActiveBrush : InactiveBrush;
        _displayExpandButton.Background = _displayButton.Background;
        UpdateTileForeground(_displayButton, _displayExpandButton, displayState.ActiveDisplays > 0);
        Grid.SetColumn(displayTile, 1);
        Grid.SetRow(displayTile, 1);
        quickGrid.Children.Add(displayTile);
    ...
    private Grid CreateSplitTile(string glyph, string label, string automationName, out Button mainButton, out Button expandButton, out TextBlock caption)
    {
        ...
        mainButton = new Button
        {
            ...
            // The popup focus lands on the first tile on launch; the system focus
            // rect reads as a stray black ring around the icon.
    ...
    {
        if (_bluetoothRefreshInProgress) return;
        _bluetoothRefreshInProgress = true;
        try
        {
        var isZh = App.Settings.Settings.Language == "zh-CN";
        var adapters = await BluetoothService.GetBluetoothAdaptersAsync();
        var active = adapters.FirstOrDefault(adapter => adapter.Radio.State == RadioState.On);
        if (_bluetoothAdapterSnapshotInitialized &&
            _activeBluetoothAdapterId != null &&
            adapters.All(adapter => adapter.Id != _activeBluetoothAdapterId) &&
            adapters.Count > 0)
        {
            ShowToast(
                isZh ? "当前蓝牙适配器已断开，可选择其他适配器继续使用" : "The active Bluetooth adapter was disconnected. Choose another adapter to continue.",
                isZh ? "选择" : "Choose",
                () =>
                {
        ...
        _bluetoothAdapterSnapshotInitialized = true;
        _activeBluetoothAdapterId = active?.Id;
        _bluetoothRadio = active?.Radio ?? adapters.FirstOrDefault()?.Radio ?? await BluetoothService.GetRadioAsync();
        ...
        if (_bluetoothRadio == null)
        {
            ...
            if (_bluetoothCaption != null) _bluetoothCaption.Text = isZh ? "不可用" : "Unavailable";
            return;
        }
        ...
        {
            System.Diagnostics.Debug.WriteLine($"Bluetooth refresh failed: {ex}");
        }
        finally
        {
            _bluetoothRefreshInProgress = false;
        }
    }
    ...
    private void RefreshAdapters()
    {
        ...
        if (_adapterDetailsCard?.Visibility == Visibility.Visible)
        {
            ...
            else if (_showBluetoothDetails || _showDisplayDetails) { }
    ...
    {
        var state = DisplayService.GetHdrState();
        var isZh = App.Settings.Settings.Language == "zh-CN";
        if (_displayCaption != null)
            _displayCaption.Text = state.ActiveDisplays <= 1
                ? (isZh ? "单屏" : "Single")
                : state.Extended ? (isZh ? "扩展" : "Extend") : (isZh ? "复制" : "Duplicate");
        if (_displayButton != null)
        {
            var active = state.ActiveDisplays > 0;
            _displayButton.Background = active ? ActiveBrush : InactiveBrush;
            if (_displayExpandButton != null) _displayExpandButton.Background = _displayButton.Background;
            UpdateTileForeground(_displayButton, _displayExpandButton, active);
        }
        if (_displayDuplicateButton != null && _displayExtendButton != null)
        {
            _displayDuplicateButton.Background = state.Extended ? InactiveBrush : ActiveBrush;
            _displayExtendButton.Background = state.Extended ? ActiveBrush : InactiveBrush;
            _displayDuplicateButton.Foreground = state.Extended ? (Brush)Application.Current.Resources["TextFillColorPrimaryBrush"] : new SolidColorBrush(Colors.White);
            _displayExtendButton.Foreground = state.Extended ? new SolidColorBrush(Colors.White) : (Brush)Application.Current.Resources["TextFillColorPrimaryBrush"];
        }
        if (_displayHdrButton != null)
        {
            _displayHdrButton.Content = state.Enabled ? (isZh ? "关闭 HDR" : "Turn HDR off") : (isZh ? "启用 HDR" : "Turn HDR on");
            _displayHdrButton.IsEnabled = state.Supported;
        }
    }

    private async Task RefreshDisplayVisualAfterDelayAsync()
    {
    ...
    {
        var hwnd = WindowNative.GetWindowHandle(this);
        // Pure toggle, matching the native version: focus-based checks misfire
        // because the tray click moves focus before the toggle arrives.
    ...
    private void ShowUsbDetails()
    {
        ...
        SetExpandRotation(_displayExpandButton, 0);
    ...
    {
        if (_adapterDetailsCard == null) return;
        if (_adapterDetailsCard.Visibility == Visibility.Visible && !_showUsbDetails && !_showBluetoothDetails && !_showDisplayDetails && _showWirelessDetails == wireless)
        {
            _adapterDetailsCard.Visibility = Visibility.Collapsed;
    ...
    {
        if (_adapterDetailsCard == null || _networkExtrasPanel == null) return;
        if (_adapterDetailsCard.Visibility == Visibility.Visible && _showDisplayDetails)
        {
            _adapterDetailsCard.Visibility = Visibility.Collapsed;
            _showDisplayDetails = false;
            SetExpandRotation(_displayExpandButton, 0);
        ...
        SetExpandRotation(_usbExpandButton, 0);
        SetExpandRotation(_bluetoothExpandButton, 0);
        SetExpandRotation(_displayExpandButton, 90);
        ...
        modes.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(1, GridUnitType.Star) });
        _displayDuplicateButton = new Button { Content = isZh ? "复制屏幕" : "Duplicate", HorizontalAlignment = HorizontalAlignment.Stretch };
        _displayExtendButton = new Button { Content = isZh ? "扩展屏幕" : "Extend", HorizontalAlignment = HorizontalAlignment.Stretch };
        var duplicate = _displayDuplicateButton;
        var extend = _displayExtendButton;
        void UpdateModeSelection(bool isExtended)
        {
            duplicate.Background = isExtended ? InactiveBrush : ActiveBrush;
            extend.Background = isExtended ? ActiveBrush : InactiveBrush;
            duplicate.Foreground = isExtended ? (Brush)Application.Current.Resources["TextFillColorPrimaryBrush"] : new SolidColorBrush(Colors.White);
            extend.Foreground = isExtended ? new SolidColorBrush(Colors.White) : (Brush)Application.Current.Resources["TextFillColorPrimaryBrush"];
        }
        UpdateModeSelection(state.Extended);
        duplicate.Click += (_, _) =>
        {
            var ok = DisplayService.SwitchMode(false);
            ShowToast(ok ? (isZh ? "已切换为复制屏幕" : "Duplicate displays enabled") : (isZh ? "切换失败" : "Switch failed"));
            if (ok) { UpdateModeSelection(false); _ = RefreshDisplayVisualAfterDelayAsync(); }
        };
        extend.Click += (_, _) =>
        {
            var ok = DisplayService.SwitchMode(true);
            ShowToast(ok ? (isZh ? "已切换为扩展屏幕" : "Extended displays enabled") : (isZh ? "切换失败" : "Switch failed"));
            if (ok) { UpdateModeSelection(true); _ = RefreshDisplayVisualAfterDelayAsync(); }
        };
        Grid.SetColumn(extend, 1);
        modes.Children.Add(duplicate);
        modes.Children.Add(extend);
        panel.Children.Add(modes);
        var layout = new Button { Content = isZh ? "屏幕布局设置" : "Display layout", HorizontalAlignment = HorizontalAlignment.Stretch };
        layout.Click += (_, _) =>
        {
            if (!DisplayService.OpenLayout()) ShowToast(isZh ? "无法打开系统屏幕设置" : "Could not open display settings");
        };
        panel.Children.Add(layout);
        _displayHdrButton = new Button { Content = state.Enabled ? (isZh ? "关闭 HDR" : "Turn HDR off") : (isZh ? "启用 HDR" : "Turn HDR on"), IsEnabled = state.Supported, HorizontalAlignment = HorizontalAlignment.Stretch };
        var hdr = _displayHdrButton;
        hdr.Click += (_, _) =>
        {
            var current = DisplayService.GetHdrState();
            var ok = DisplayService.SetHdr(!current.Enabled);
            ShowToast(ok ? (isZh ? "HDR 状态已更新" : "HDR state updated") : (isZh ? "HDR 操作失败或当前屏幕不支持" : "HDR failed or unsupported"));
            var actual = DisplayService.GetHdrState();
            hdr.Content = actual.Enabled ? (isZh ? "关闭 HDR" : "Turn HDR off") : (isZh ? "启用 HDR" : "Turn HDR on");
            hdr.IsEnabled = actual.Supported;
            RefreshDisplayVisual();
        };
    ...
    private async Task PopulateBluetoothDevicesAsync()
    {
        ...
        var refresh = new Button { Content = isZh ? "扫描中…" : "Scanning…", IsEnabled = false, MinWidth = 70 };
        ...
        {
            Text = isZh ? "蓝牙适配器（同一时间仅启用一个）" : "Bluetooth adapter (one active at a time)",
        ...
        {
            var active = adapters.FirstOrDefault(adapter => adapter.Radio.State == RadioState.On);
            var selectorRow = new Grid { ColumnSpacing = 8 };
            ...
            {
                ItemsSource = adapters,
                DisplayMemberPath = nameof(BluetoothService.AdapterEntry.Name),
                SelectedItem = active ?? adapters[0],
                HorizontalAlignment = HorizontalAlignment.Stretch
            };
            var switchButton = new Button { MinWidth = 70 };
            void UpdateSwitchButton()
            {
                var selected = selector.SelectedItem as BluetoothService.AdapterEntry;
                switchButton.Content = selected?.Radio.State == RadioState.On
                    ? (isZh ? "禁用" : "Disable")
                    : (isZh ? "切换" : "Switch");
                switchButton.IsEnabled = selected != null;
            }
            selector.SelectionChanged += (_, _) => UpdateSwitchButton();
            switchButton.Click += async (_, _) =>
            {
                if (selector.SelectedItem is not BluetoothService.AdapterEntry selected) return;
                switchButton.IsEnabled = false;
                var wasOn = selected.Radio.State == RadioState.On;
                var success = wasOn
                    ? await BluetoothService.SetEnabledAsync(selected.Radio, false)
                    : await BluetoothService.SwitchBluetoothAdapterAsync(selected, adapters);
                ShowToast(success
                    ? (wasOn
                        ? (isZh ? $"已禁用 {selected.Name}" : $"Disabled {selected.Name}")
                        : (isZh ? $"已切换到 {selected.Name}" : $"Switched to {selected.Name}"))
                    : (isZh ? "蓝牙适配器操作失败" : "Bluetooth adapter operation failed"));
                await RefreshBluetoothAsync();
            ...
            selectorRow.Children.Add(switchButton);
            target.Children.Add(selectorRow);
        }
    ...
    private void PopulateAdapterDetails(bool wireless)
    {
        ...
        if (wireless) _selectedWirelessAdapter = _selectedAdapter;
        UpdateSelectedAdapterInfo();
        _adapterListPanel?.Children.Clear();
        _networkExtrasPanel?.Children.Clear();
        if (_adapterExpander != null)
            _adapterExpander.Header = _selectedAdapter == null
        ...
        if (_adapterListPanel != null && adapters.Count > 0 && !wireless)
        {
            ...
            {
                HorizontalAlignment = HorizontalAlignment.Stretch,
                DisplayMemberPath = nameof(NetworkAdapter.Name),
                ItemsSource = adapters,
                SelectedItem = _selectedAdapter
            };
            var toggle = new Button
            {
                Content = _selectedAdapter?.IsUp == true ? (isZh ? "禁用" : "Disable") : (isZh ? "启用" : "Enable"),
                MinWidth = 64
            };
            selector.SelectionChanged += (_, _) =>
            {
                if (selector.SelectedItem is not NetworkAdapter selected) return;
                _selectedAdapter = selected;
                if (wireless) _selectedWirelessAdapter = selected;
                toggle.Content = selected.IsUp ? (isZh ? "禁用" : "Disable") : (isZh ? "启用" : "Enable");
                if (_adapterExpander != null)
                    _adapterExpander.Header = $"{(isZh ? "网络适配器" : "Network adapter")} · {selected.Name}";
                UpdateSelectedAdapterInfo();
                if (wireless && _networkExtrasPanel != null)
                {
                    _networkExtrasPanel.Children.Clear();
                    _networkExtrasPanel.Children.Add(CreateWifiNetworkControls());
                }
            };
            toggle.Click += async (_, _) =>
            {
                if (selector.SelectedItem is not NetworkAdapter selected || _isOperationInProgress) return;
                _isOperationInProgress = true;
                toggle.IsEnabled = false;
                var success = selected.IsUp
                    ? await NetworkService.DisableAdapterAsync(selected.InterfaceName)
                    : await NetworkService.EnableAdapterAsync(selected.InterfaceName);
                _networkService.RefreshAdapters();
                ShowToast(success
                    ? (isZh ? "网卡状态已更新" : "Adapter state updated")
                    : (isZh ? "网卡操作失败，请检查管理员权限" : "Adapter operation failed; check administrator privileges"));
                _isOperationInProgress = false;
                PopulateAdapterDetails(wireless);
            };
        ...
        else if (_adapterListPanel != null && !wireless)
        {
            ...
            {
                Text = isZh ? "未检测到可用网络适配器" : "No network adapter detected",
                Opacity = 0.7
            });
        }
    ...
    private async Task ShowConnectionPriorityManagerAsync(bool isZh)
    {
        _networkService.RefreshAdapters();
        var adapters = _networkService.Adapters
            .Where(adapter => adapter.IsEthernet || adapter.IsWireless || adapter.IsUsbTethering)
            .GroupBy(adapter => adapter.Id)
            .Select(group => group.First())
            .ToList();
        if (adapters.Count == 0)
        {
            ShowToast(isZh ? "未检测到可调整优先级的网卡" : "No adapters available for priority management");
            return;
        }

        var ranked = new List<(NetworkAdapter Adapter, int Metric)>();
        foreach (var adapter in adapters)
            ranked.Add((adapter, await NetworkService.GetInterfaceMetricAsync(adapter.InterfaceName) ?? 9999));
        ranked.Sort((left, right) => left.Metric.CompareTo(right.Metric));
        ...
        foreach (var item in ranked)
        {
            list.Items.Add(new ListViewItem
            {
                ...
                {
                    Text = $"{item.Adapter.Name}  ·  metric {item.Metric}",
        ...
        {
            Text = isZh ? "拖动网卡调整顺序。越靠上优先级越高，应用后自动配置 metric。" : "Drag adapters to reorder them. Higher entries get higher priority and metrics are assigned automatically.",
            TextWrapping = TextWrapping.Wrap,
            Opacity = 0.72
        });
        ...
        {
            XamlRoot = (Content as FrameworkElement)?.XamlRoot,
            Title = isZh ? "上网方式优先级" : "Connection priority",
            Content = content,
        ...
        var priorities = list.Items.OfType<ListViewItem>()
            .Select((item, index) => (((NetworkAdapter)item.Tag).InterfaceName, 10 + index * 10))
            .ToList();
        var success = await NetworkService.SetInterfacePrioritiesAsync(priorities);
        ShowToast(success
            ? (isZh ? "已按新顺序自动配置网卡 metric" : "Adapter metrics updated from the new order")
            : (isZh ? "优先级应用失败，请检查管理员权限" : "Failed to apply priorities; check administrator privileges"));
    }
    ...
    private void ConfigureBackdropTheme(ElementTheme theme)
    {
        ...
        // Keep the material closer to the shell's quick settings acrylic: the
        // previous lower opacities let the desktop show through too strongly.
        _acrylicController.TintOpacity = dark ? 0.62f : 0.55f;
        _acrylicController.LuminosityOpacity = dark ? 0.72f : 0.78f;
    ...
    private void BuildSettingsView()
    {
        ...
        {
            Content = isZh ? "管理" : "Manage",
            MinWidth = 76,
            VerticalAlignment = VerticalAlignment.Center
        };
        priorityButton.Click += async (_, _) => await ShowConnectionPriorityManagerAsync(isZh);
        CardsPanel.Children.Add(CreateSettingsCard(
            "\uE839", isZh ? "上网方式优先级" : "Connection priority",
            isZh ? "拖动调整网卡顺序并自动配置 metric" : "Drag adapters to reorder and assign metrics automatically",
            priorityButton, cardBackground, cardBorder));
    ...
    private static class NativeMethods
    {
        ...
        [System.Runtime.InteropServices.StructLayout(System.Runtime.InteropServices.LayoutKind.Sequential)]
        public struct NATIVE_RECT { public int Left, Top, Right, Bottom; }
        [System.Runtime.InteropServices.StructLayout(System.Runtime.InteropServices.LayoutKind.Sequential)]
        public struct MONITORINFO { public uint Size; public NATIVE_RECT Monitor; public NATIVE_RECT Work; public uint Flags; }
